...
class SpeedLoggingSFTTrainer(SFTTrainer):
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        if self._window_steps >= self.log_interval:
            ...
            # Trainer 내부 logger로 기록 → log, JSON, TensorBoard 등에 같이 남음
            self.log({
...
class Qwen14BFineTuner:
    ...
    def load_model(self):
        ...
        # Flash Attention 3 시도, 실패 시 Flash Attention 2, 최종적으로 Unsloth가 자동으로 fallback
        logger.info("[ INFO ] Attention 구현: Flash Attention 3 시도 (실패 시 Flash Attention 2, 최종적으로 Unsloth 자동 선택)")
        ...
        try:
            ...
            self.model, self.tokenizer = FastLanguageModel.from_pretrained(
                ...
                attn_implementation="flash_attention_3"  # Flash Attention 3 시도
            )
    ...
    def train(self, dataset: Dataset) -> str:
        ...
        # Trainer 생성
        logger.info("[ INFO ] SFTTrainer 초기화 중... (SpeedLogging 포함)")
        trainer = SpeedLoggingSFTTrainer(
            ...
            args=training_args,
            callbacks=callbacks,
            # 추가: 몇 step마다 속도 로그를 찍을지
            log_interval=10,  # 10 step마다 속도 로그
        )
    ...
    def _find_resume_checkpoint(self, logger) -> Optional[str]:
        ...
        if os.path.exists(self.config.output_dir):
            ...
            checkpoints = [d for d in os.listdir(self.config.output_dir) 
                          if d.startswith("checkpoint-") and os.path.isdir(os.path.join(self.config.output_dir, d))]
            if checkpoints:
                # 체크포인트 유효성 검증: trainer_state.json 파일이 있어야 학습 재개 가능
                def is_valid_checkpoint(checkpoint_name: str) -> bool:
                    """체크포인트가 학습 재개 가능한지 확인 (trainer_state.json 존재 여부)"""
                    checkpoint_path = os.path.join(self.config.output_dir, checkpoint_name)
                    trainer_state_path = os.path.join(checkpoint_path, "trainer_state.json")
                    return os.path.exists(trainer_state_path)
                
                # 유효한 체크포인트만 필터링
                valid_checkpoints = [c for c in checkpoints if is_valid_checkpoint(c)]
                
                if not valid_checkpoints:
                    invalid_checkpoints = [c for c in checkpoints if not is_valid_checkpoint(c)]
                    if invalid_checkpoints:
                        logger.warning(f"[ CHECKPOINT ] ⚠️  체크포인트 디렉토리는 있지만 trainer_state.json이 없습니다:")
                        for c in invalid_checkpoints:
                            logger.warning(f"  - {c} (학습 재개 불가능, 처음부터 시작)")
                        logger.info("[ CHECKPOINT ] 유효한 체크포인트가 없어 처음부터 학습을 시작합니다.")
                        return None
                
                # Hub 업로드 완료된 checkpoint 우선 선택
                uploaded_checkpoints = [c for c in valid_checkpoints 
                ...
                if uploaded_checkpoints:
                    ...
                    latest_checkpoint = os.path.join(self.config.output_dir, valid_checkpoints[-1])
                    step_num = valid_checkpoints[-1].split('-')[1]
